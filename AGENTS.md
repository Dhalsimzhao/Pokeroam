...
Special cases:
- `execute and verify` / `执行并验证` — run execution first, then verification sequentially.
- `plan` / `规划` used **without** a preceding `design` step — enters **fast mode** (see below).
...
There are two workflow modes for executing tasks from the Scrum:

#### Full Mode (complex tasks)

Trigger: start with `design` / `设计`

```
design → plan → summarize → execute → verify
```

5 steps. Use for tasks involving multiple files, architectural decisions, cross-component changes, or unfamiliar code areas. Each step produces its own document (Task.md → Planning.md → Execution.md).

#### Fast Mode (simple tasks)

Trigger: start with `plan` / `规划` directly (skipping `design`)

```
plan → execute and verify
```

2 steps. Use for tasks with clear scope, few files, straightforward implementation, or well-understood patterns. The `plan` step:
1. Reads the task from Scrum.md, marks it as `[x]`.
2. Does lightweight analysis (no separate Task.md design document).
3. Produces Execution.md directly (merging design + plan + summarize into one).

Then `execute and verify` / `执行并验证` applies changes and verifies in one step.

**When to use which:**

| Fast Mode | Full Mode |
|-----------|-----------|
| 1-2 files changed | 3+ files or cross-component |
| Implementation is obvious from Scrum | Needs architectural analysis |
| Following established patterns | Introducing new patterns |
| Pure logic / data changes | New windows, IPC, UI components |
