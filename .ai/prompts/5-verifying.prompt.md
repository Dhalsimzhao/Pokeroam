...
- All instructions in `Execution.md` should have been applied to the source code. Your goal is to verify it.
- You must ensure the source code compiles (type checks pass).
- You must ensure all tests pass (if applicable).
- Until everything passes, ensure there is a `# !!!VERIFIED!!!` mark at the end of `Execution.md`.

## Step 1. Check and Respect My Code Changes

- If you spot any difference between `Execution.md` and the source code:
  - **It means I edited them. I have my reason. DO NOT change the code to match `Execution.md`.**
  - Write down every difference you spotted, make a `## User Update Spotted` section in the `# UPDATES` section in `Execution.md`.
...
- Check `# AFFECTED PROJECTS` in `Execution.md` for which tests to run.
- Make sure any newly added test cases are actually executed.

#### Identify the Cause of Failure

- Refer to `Task.md` and `Planning.md` for context, keep the target unchanged.
...
- When everything passes, add `# !!!VERIFIED!!!` to the end of `Execution.md`.
